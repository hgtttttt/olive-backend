...
class Users:
        ...
        if request.method == "POST":
            ...
            if username is not None and password is not None:
                ...
                if islogin is not None:
                    ...
                    #     avatar = "https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png"
                    # avatar = userprofile.avatar
                    tp = Profile.objects.filter(user_id=user_id).first()
                    tt = Team.objects.filter(user_id=user_id).first()
                    if tp is None and tt is not None:
                        return JsonResponse({
                            "error_code": 0,
                            "data": {
                                "id": tt.id,
                    ...
                    elif tp is not None and tt is None:
                        return JsonResponse({
                            "error_code": 0,
                            "data": {
                                "id": tp.user.id,
                    ...
                    elif tp is not None and tt is not None:
                        return JsonResponse({
                            "error_code": 3,
                            "message": "Inconsistency error, all not None"
                        })
                    else:
                        return JsonResponse({
                            "error_code": 3,
                            "message": "Inconsistency error, all None"
                        })
                else:
                    return JsonResponse({
                        "error_code": 1,
    ...
    #         if user is None:
    #             return JsonResponse({"error_code": 1, "message": "user not exist"})
    #         username = user.username
    #         # print(password)
    ...
    def modify_password(request):
        ...
        if request.method == "POST":
            ...
            newpassword = data.get("newpass")
            if type == 1:
                user = Team.objects.filter(pk=id).first().user
            else:
                user = User.objects.get(id=id)
            if user is None:
                return JsonResponse({"error_code": 1, "message": "user not exist"})
...
class Personality:
    ...
    def change_personality(request):
        ...
        if request.method == 'POST':
            ...
            profile.phone = phone
            # profile.bio = bio
            # profile.email = data.get("org")
            profile.address = data.get("addr")
